...
def invoice(initial_energy_lecture, final_energy_lectures, monthly_comsumn_energy, monthly_cost, initial_datetime, final_datetime, actual_datetime, client_num, client_first_name, client_last_name, client_address):
    ...
    connecting_database = sqlite3.connect("./energy_consumption.db")
    ...
    plt.grid(True)
    plt.savefig(f"Client:{client_num}/{actual_month}.png")  # Save the graph as an image
    plt.close()
    ...
    invoice_pdf.output(f"Client N°{client_num}_{actual_month}_electric_bill.pdf")
